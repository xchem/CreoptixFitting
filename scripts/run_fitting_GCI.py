...
assert len(analyte_keys_included) == len(calibration_keys_included), print("Please check both provided keys.")
...
if args.global_fitting:
    init_expts = []
    for analyte_keys, calibration_keys in zip(analyte_keys_included, calibration_keys_included):
        init_expts.append(_load_gci_infor_from_argument(analyte_keys, calibration_keys, args))
    ...
    for analyte_keys, calibration_keys in zip(analyte_keys_included, calibration_keys_included):
        ...
        init_expts.append(_load_gci_infor_from_argument(analyte_keys, calibration_keys, args))
        experiments = _extract_GCI(init_expts, args)
        _, params_hat = fitting_expts(experiments, os.path.join(args.out_dir, analyte_keys[3:]), args)
        del init_expts