...
def convert_experiments_to_jax_format(experiments):
    ...
    jax_compatible_experiments = {
        key: jnp.array([exp[key] for exp in experiments]) for key in keys
    }
    return jax_compatible_experiments


def _fitting_each_GCI_dataset_deterministic(
    experiment, params, return_conc=False, fitting_specific=False
):
    """
    ...
    if fitting_specific:
        ...
        Rt_model = _jax_Rt_model_complex(
            Rt=Rt,
            R_CL=R_CL,
            cLt=cLt,
            ts=ts,
            ka_P=ka_P,
            kd_P=kd_P,
            Rmax_P=params["Rmax_P"],
            ka_C=ka_C,
            kd_C=kd_C,
            Rmax_C=params["Rmax_C"],
            alpha=params["alpha"],
...
def _fitting_expts_simple_binding(experiments, args=None):
    ...
    Rt_models = jax.vmap(deterministic_step)(
        experiments_jax, jnp.arange(len(experiments))
    )

    # Vectorized calculation of Normal distribution
    Rt_model_to_fit = jax.vmap(lambda R, mask: jnp.where(mask, R, 0))(
        Rt_models, experiments_jax["to_fit"]
    )
    Rt_to_fit = jax.vmap(lambda R, mask: jnp.where(mask, R, 0))(
        experiments_jax["Rt"], experiments_jax["to_fit"]
    )
    ...
    numpyro.sample(
        "R_obs",
        dist.Normal(loc=Rt_model_to_fit, scale=jnp.exp(log_sigmas_expanded)),
        obs=Rt_to_fit,
    )
...
def _fitting_expts_complex(experiments, priors={}, args=None):
    ...
    for idx, experiment in enumerate(experiments):
        ...
        numpyro.sample(
            f"R_obs{idx:02d}",
            dist.Normal(loc=Rt_model_to_fit, scale=jnp.exp(log_sigmas[idx])),
            obs=Rt_to_fit,
        )
