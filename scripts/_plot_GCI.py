...
def _Rt_model(cL_ts, ts, ka, kd, Rmax, rate_decay):
    ...
    """
    Rmax_adj = np.exp(np.log(Rmax) + np.log(1. - rate_decay)*ts/60)
...
def plot_Rt_dRdt(experiment, params, t_cutoff=None, fig_name=None, OUTFILE=None):
    ...
    if 'ts_peak' in experiment.keys():
        ...
        for _ts_peak in ts_peak:
            ...
            ax1.plot(ts[fit_plot], dRdt_hat[fit_plot], '-', label="Fitting", color='orange',
                    linewidth=3.0)
            handles, labels = ax1.get_legend_handles_labels()
    else:
...
def plot_Creoptix_Bayesian(experiment, MAP, params_mean_std, ax=None, xlim=None, ylim=None,
                           fig_name=None, OUTFILE=None):
    """
    ...
    if params_mean_std is not None:
        [ka_hat, kd_hat, Kd_hat, Rmax_hat, y_offset_hat] = [params_mean_std['ka'], params_mean_std['kd'], params_mean_std['Kd'],
                                                            params_mean_std['Rmax'], params_mean_std['y_offset']]
        [ka, kd, Kd, Rmax, y_offset] = [ka_hat.n, kd_hat.n, Kd_hat.n*1E6, Rmax_hat.n, y_offset_hat.n]
        [ka_std, kd_std, Kd_std, Rmax_std, y_offset_std] = [ka_hat.s, kd_hat.s, Kd_hat.s*1E6, Rmax_hat.s, y_offset_hat.s]
        text = r'$R_{max}$: '+ str('%2.2f' %Rmax) + r' $\pm$ '+ str('%2.2E' %Rmax_std) + str(' (pg/mm$^2$)')
        text += '\n$k_a$: '+ str('%2.2E' %ka) + r' $\pm$ '+ str('%2.2E' %ka_std) + str(' $(M^{-1}s^{-1})$')
        text += '\n$k_d$: '+ str('%2.2E' %kd) + r' $\pm$ '+ str('%2.2E' %kd_std) + str(' (s$^{-1}$)')
        text += '\n$K_d$: '+ str('%2.2f' %Kd) + r' $\pm$ '+ str('%2.2f' %Kd_std) + str(' $\mu$M')
        if y_offset != 0:
            text += '\n$y_{offset}$: '+ str('%2.2f' %y_offset) + r' $\pm$ '+ str('%2.2f' %y_offset_std)
    else:
    ...
    if 'injection' in experiment.keys():
        for inj in experiment['injection']:
            ax.vlines(inj[0], ymin=min(Rt[to_fit]), ymax=max(Rt_hat), ls='--', color='k', alpha=0.5)
...
def _Rt_model_specific(cLt, ts, ka_P, kd_P, Rmax_P, ka_C, kd_C, Rmax_C, rate_decay, alpha, epsilon, R_CL):
    """
    Return the GCI sensogram given the parameters
    """
    Rmax_P_adj  = np.exp(np.log(Rmax_P) + np.log(1. - rate_decay)*ts/60)

    _cLt     = lambda t: np.interp(t, ts, cLt)
    _Rmaxt   = lambda t: np.interp(t, ts, Rmax_P_adj)
    dR_PLdt  = lambda R_PL, t: ka_P*_cLt(t)*(_Rmaxt(t) - R_PL) - kd_P*R_PL + epsilon
...
def plot_Rt_dRdt_complex(experiment, params, t_cutoff=None, fig_name=None, no_subtraction=False, OUTFILE=None):
    """
    ...
    if experiment['binding_type'] == 'non_specific':
        [ka_C, kd_C, Rmax_C, y_offset] = [params['ka_C'], params['kd_C'], params['Rmax_C'], params['y_offset']]
        _params_NSB = {'ka': ka_C, 'kd': kd_C, 'Rmax': Rmax_C, 'y_offset': y_offset}
        plot_Rt_dRdt(experiment=experiment, params=_params_NSB, t_cutoff=t_cutoff, 
                     fig_name=fig_name, OUTFILE=OUTFILE)
    else:
        [ka_P, kd_P, Rmax_P, ka_C, kd_C, Rmax_C] = [params['ka_P'], params['kd_P'], params['Rmax_P'], 
                                                    params['ka_C'], params['kd_C'], params['Rmax_C']]
        [rate_decay, alpha, epsilon, y_offset]   = [params['rate_decay'], params['alpha'], params['epsilon'], params['y_offset']]
        ...
        if no_subtraction:
            ...
            Rt_hat = _Rt_model_specific(cLt, ts, ka_P, kd_P, Rmax_P, ka_C, kd_C, Rmax_C, rate_decay, alpha, epsilon, Rt_CL) + y_offset + Rt_CL
            dRdt   = np.concatenate(([0], np.diff(Rt)/np.diff(ts)))
            dRdt_hat = np.concatenate(([0], np.diff(Rt_hat)/np.diff(ts)))
        else:
            Rt     = experiment['Rt']
            Rt_CL  = experiment['Rt_FC1']
            Rt_hat = _Rt_model_specific(cLt, ts, ka_P, kd_P, Rmax_P, ka_C, kd_C, Rmax_C, rate_decay, alpha, epsilon, Rt_CL) + y_offset
            dRdt   = experiment['dRdt']
            dRdt_hat = np.concatenate(([0], np.diff(Rt_hat)/np.diff(ts)))
...
def plot_Creoptix_Bayesian_complex(experiment, MAP, params_mean_std, ax=None, xlim=None, ylim=None,
                                   fig_name=None, no_subtraction=False, OUTFILE=None):
    """
    Plotting the fitted and the observed sensorgrams for non-specific model
    """
    if experiment['binding_type'] == 'non_specific':
        [ka_C, kd_C, Rmax_C, y_offset] = [MAP['ka_C'], MAP['kd_C'], MAP['Rmax_C'], MAP['y_offset']]
        _MAP_NSB = {'ka': ka_C, 'kd': kd_C, 'Rmax': Rmax_C, 'y_offset': y_offset}
        _param_mean_std_NSB = {'ka': params_mean_std['ka_C'],
                               'kd': params_mean_std['kd_C'],
                               'Kd': params_mean_std['Kd_C'],
                               'Rmax': params_mean_std['Rmax_C'],
                               'y_offset': params_mean_std['y_offset']
                              }
        plot_Creoptix_Bayesian(experiment=experiment, MAP=_MAP_NSB, params_mean_std=_param_mean_std_NSB,
                               ax=ax, xlim=xlim, ylim=ylim, fig_name=fig_name, OUTFILE=OUTFILE)
    else:
        [ka_P, kd_P, Rmax_P, ka_C, kd_C, Rmax_C] = [MAP['ka_P'], MAP['kd_P'], MAP['Rmax_P'],
                                                    MAP['ka_C'], MAP['kd_C'], MAP['Rmax_C']]
        [rate_decay, alpha, epsilon, y_offset]   = [MAP['rate_decay'], MAP['alpha'], MAP['epsilon'], MAP['y_offset']]
        ...
        if params_mean_std is not None:
            [ka_hat, kd_hat, Kd_hat, Rmax_hat, y_offset_hat] = [params_mean_std['ka_P'], params_mean_std['kd_P'], params_mean_std['Kd_P'],
                                                                params_mean_std['Rmax_P'], params_mean_std['y_offset']]
            [ka, kd, Kd, Rmax, y_offset] = [ka_hat.n, kd_hat.n, Kd_hat.n*1E6, Rmax_hat.n, y_offset_hat.n]
            [ka_std, kd_std, Kd_std, Rmax_std, y_offset_std] = [ka_hat.s, kd_hat.s, Kd_hat.s*1E6, Rmax_hat.s, y_offset_hat.s]
            text = r'$R_{max}$: '+ str('%2.2f' %Rmax) + r' $\pm$ '+ str('%2.2E' %Rmax_std) + str(' (pg/mm$^2$)')
            text += '\n$k_a$: '+ str('%2.2E' %ka) + r' $\pm$ '+ str('%2.2E' %ka_std) + str(' $(M^{-1}s^{-1})$')
