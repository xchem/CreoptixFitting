...
def _report_params(
    trace, experiments, map_index, return_conc=False, return_y_offset=False
):
    """
    Parameters:
    ----------
    ...
    params_hat["ka"] = ufloat(
        jnp.mean(jnp.exp(trace["logka"])), jnp.std(jnp.exp(trace["logka"]))
    )

    if "logKeq" in trace.keys():
        Keq_hat = ufloat(
            jnp.mean(jnp.exp(trace["logKeq"])), jnp.std(jnp.exp(trace["logKeq"]))
        )
        ...
        Kd_hat = ufloat(
            jnp.mean(jnp.exp(trace["logKd"])), jnp.std(jnp.exp(trace["logKd"]))
        )
        Kd_hat = Kd_hat
    ...
    for idx in range(len(experiments)):
        params_hat[f"Rmax:{idx:02d}"] = ufloat(
            jnp.mean(trace[f"Rmax:{idx:02d}"]), jnp.std(trace[f"Rmax:{idx:02d}"])
        )

        if f"y_offset:{idx:02d}" in trace.keys():
            y_offset_hat = ufloat(
                jnp.mean(trace[f"y_offset:{idx:02d}"]),
                jnp.std(trace[f"y_offset:{idx:02d}"]),
            )
        else:
            y_offset_hat = ufloat(0.0, 0.0)
        params_hat[f"y_offset:{idx:02d}"] = y_offset_hat

    return params, params_hat


def _report_params_complex(
    trace, experiments, map_index, return_conc=False, return_y_offset=False
):
    """
    Parameters:
    ...
    params_hat["ka_P"] = ufloat(
        jnp.mean(jnp.exp(trace["logka_P"])), jnp.std(jnp.exp(trace["logka_P"]))
    )

    if "logKeq_P" in trace.keys():
        Keq_hat = ufloat(
            jnp.mean(jnp.exp(trace["logKeq_P"])), jnp.std(jnp.exp(trace["logKeq_P"]))
        )
        ...
        Kd_hat = ufloat(
            jnp.mean(jnp.exp(trace["logKd_P"])), jnp.std(jnp.exp(trace["logKd_P"]))
        )
        Kd_hat = Kd_hat
    ...
    params_hat["ka_C"] = ufloat(
        jnp.mean(jnp.exp(trace["logka_C"])), jnp.std(jnp.exp(trace["logka_C"]))
    )

    if "logKeq_C" in trace.keys():
        Keq_hat = ufloat(
            jnp.mean(jnp.exp(trace["logKeq_C"])), jnp.std(jnp.exp(trace["logKeq_C"]))
        )
        ...
        Kd_hat = ufloat(
            jnp.mean(jnp.exp(trace["logKd_C"])), jnp.std(jnp.exp(trace["logKd_C"]))
        )
        Kd_hat = Kd_hat
    ...
    for idx in range(len(experiments)):
        for key in ["Rmax_P", "rate_decay", "alpha", "epsilon", "y_offset"]:
            if f"{key}:{idx:02d}" in trace.keys():
                params_hat[f"{key}:{idx:02d}"] = ufloat(
                    jnp.mean(trace[f"{key}:{idx:02d}"]),
                    jnp.std(trace[f"{key}:{idx:02d}"]),
                )

        if not f"y_offset:{idx:02d}" in params_hat.keys():
            params_hat[f"y_offset:{idx:02d}"] = ufloat(0.0, 0.0)
