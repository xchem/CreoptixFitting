...
N_cycles = len(cycles)
assert N_concs==N_cycles, print("Incorrect input. There is f{N_concs} analyte concentrations while f{N_cycles} cycles.")
...
if len(cycles)>0:
    for idx, j in enumerate(cycles):
        ...
        if args.fitting_subtract:
            for ith_analyte, ith_calib in zip(analyte_keys_included, calibration_keys_included):
        ...
        _key_included = [item if isinstance(item, str) else item[0] for item in _key_included]
        _calibration_keys_included = [item if isinstance(item, str) else item[0] for item in _calibration_keys_included]
...
cd ''' + args.out_dir + '''\n''' + \
    	...
    	''' --out_dir ''' + out_dir + \
    	global_fitting + fitting_complex + fitting_subtract + \
        include_protein_decay + return_conc + return_y_offset + return_epsilon + \
        ...
        ''' --analyte_file ''' + args.analyte_file + \
