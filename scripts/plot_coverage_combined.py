...
if len(args.mcmc_dir)>0:
    ...
    if len(b_rates_list) == 0: 
        for param in params:
            ...
            if param.startswith('logkd'): param_name = param+'_1'
            ...
            if os.path.isfile(f"{param_name[3:]}.pkl"):
                ...
                if param == 'logkd':
                    ...
                    samples = [jnp.exp(pickle.load(open(os.path.join(args.mcmc_dir, name, "traces.pickle"), "rb"))[param]) for name in experiment_names]
                ...
                for level in LEVELS:
                    print(level)
                    rate, rate_error = rate_of_containing_from_sample(samples=samples, level=level, estimate_of_true=args.central,
                                                                      true_val = b_centrals,
                                                                      ci_type="bayesian", bootstrap_repeats=100)
                    rate *= 100
                    rate_error *= 100
...
if len(args.nonlinear_fit_result_file)>0:
    ...
    pickle.dump({"LEVELS_PERCENT": LEVELS_PERCENT, "g_rates": g_rate_list}, open("NLS.pkl", "wb"))
...
for i, param in enumerate(['ka', 'kd', 'Kd']):
    ...
    if len(args.mcmc_dir)>0:
        plot_containing_rates([LEVELS_PERCENT], [b_rates_list[param]],
    ...
    if len(args.nonlinear_fit_result_file)>0:
        plot_containing_rates([LEVELS_PERCENT], [g_rate_list[param]],
                              observed_rate_errors=None,
                              xlabel=XLABEL, ylabel=YLABEL, 
                              xlimits=[0, 100], ylimits=[0, 100], markers=MARKERS[1], 
                              ax=axes[i], colors=COLORS[1], label='NLS')
        handles, labels = axes[i].get_legend_handles_labels()
...
legend = fig.legend(by_label.values(), by_label.keys(), loc='center right', bbox_to_anchor=(1.15, 0.6));
