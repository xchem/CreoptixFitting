...
def _prior_GCI(experiments, args=None):
    ...
    # Global parameters
    params['logKd'] = uniform_prior(name='logKd', lower=-35.5, upper=0.)
    params['logka'] = uniform_prior(name='logka', lower=4.5, upper=20.)
    ...
    for idx, experiment in enumerate(experiments):
        ...
        Rmax_array = Rmax_array.at[idx].set(uniform_prior(name=f'Rmax:{idx:02d}', lower=0.2*maxR[idx], upper=100*maxR[idx]))

        # Sample y_offset as a local parameter if specified
        if args.return_y_offset:
            y_offset_array = y_offset_array.at[idx].set(uniform_prior(name=f'y_offset:{idx:02d}', lower=minR[idx]-10, upper=minR[idx]+10))
        else:
            y_offset_array = y_offset_array.at[idx].set(0.0)

        if args.include_protein_decay:
            protein_decay_array = protein_decay_array.at[idx].set(uniform_prior(name=f'rate_decay:{idx:02d}', lower=0., upper=0.5))

        # Sample y_offset as a local parameter
        log_sigma_min, log_sigma_max = logsigma_guesses(experiment['Rt'])
        log_sigma_Rt_array = log_sigma_Rt_array.at[idx].set(uniform_prior(name=f'log_sigma_Rt:{idx:02d}', lower=log_sigma_min, upper=log_sigma_max))
    ...
    if args.return_conc:
        conc_uM = experiments[0]['analyte_concentration'] * 1E6  # Assuming the same concentration for simplicity
        dE = 0.1
...
def _prior_GCI_complex(experiments, priors={}, args=None):
    ...
    # Global parameters
    params['logKd_P'] = uniform_prior(name='logKd_P', lower=-35.5, upper=0.)
    params['logka_P'] = uniform_prior(name='logka_P', lower=4.5, upper=20.)

    if not 'logKd_C' in priors.keys(): params['logKd_C'] = uniform_prior(name='logKd_C', lower=-35.5, upper=0.)
    else: params['logKd_C'] = priors['logKd_C']

    if not 'logka_C' in priors.keys(): params['logka_C'] = uniform_prior(name='logka_C', lower=4.5, upper=20.)
    else: params['logka_C'] = priors['logka_C']

    if not 'Rmax_C' in priors.keys(): params['Rmax_C'] = uniform_prior(name='Rmax_C', lower=0, upper=500.)
    else: params['Rmax_C'] = priors['Rmax_C']
    ...
    for idx, experiment in enumerate(experiments):
        ...
        if experiment['binding_type'] == 'specific':
            # Sample Rmax as a local parameter
            Rmax_array = Rmax_array.at[idx].set(uniform_prior(name=f'Rmax_P:{idx:02d}', lower=0.2*maxR[idx], upper=10*maxR[idx]))

            # Sample alpha as a local parameter if not specified
            if not 'alpha' in priors.keys():
                alpha_array = alpha_array.at[idx].set(uniform_prior(name=f'alpha:{idx:02d}', lower=0., upper=0.3))

            # Sample epsilon as a local parameter
            if args.return_epsilon:
                epsilon_array = epsilon_array.at[idx].set(uniform_prior(name=f'epsilon:{idx:02d}', lower=0., upper=10.))
        ...
        # Sample y_offset as a local parameter if specified
        if args.return_y_offset:
            y_offset_array = y_offset_array.at[idx].set(uniform_prior(name=f'y_offset:{idx:02d}', lower=minR[idx]-10, upper=minR[idx]+20))
...
def _prior_each_data_complex(all_params, idx, fitting_specific=False):
    ...
    """
    if fitting_specific:
        params = {'logka_P'     : all_params['logka_P'],
        ...
        params = {'logka'       : all_params['logka_C'],
