...
def _map_update_by_prior(init_trace, priors, num_experiments):
    ...
    trace.update({key: jnp.ones(N)*priors[key] for key in ['logka_C', 'logKd_C', 'Rmax_C'] if key in priors})

    if 'alpha' in priors.keys():
        for idx in range(num_experiments):
            trace[f'alpha:{idx:02d}'] = jnp.ones(N)*priors['alpha']
        trace.update({'logka_C': jnp.zeros(N), 'logKd_C': jnp.zeros(N), 'Rmax_C': jnp.zeros(N)}) if priors.get('alpha') == 1 else None
...
def _log_priors(mcmc_trace, experiments, nsamples=None):
    ...
    f_prior_ka      = vmap(lambda param: _uniform_pdf(param, 4.5, 20.))
    f_prior_kd      = vmap(lambda param: _uniform_pdf(param, 0.0, 20.0))
    f_prior_Kd      = vmap(lambda param: _uniform_pdf(param, -35.5, 0))
    f_prior_Rmax_C  = vmap(lambda param: _uniform_pdf(param, 0., 500.))
    f_prior_decay   = vmap(lambda param: _uniform_pdf(param, 0., 0.5))
    f_prior_alpha   = vmap(lambda param: _uniform_pdf(param, 0, 0.3))
    f_prior_epsilon = vmap(lambda param: _uniform_pdf(param, 0., 10.))
    ...
    if 'conc' in mcmc_trace.keys():
        ...
        dE = 0.1
        log_priors += jnp.log(_lognormal_pdf(mcmc_trace['conc'][:nsamples], conc_uM, dE*conc_uM))
    ...
    for idx, expt in enumerate(experiments):
        ...
        if f'log_sigma_Rt:{idx}' in mcmc_trace.keys():
            log_priors += _log_prior_sigma(mcmc_trace=mcmc_trace, data=expt['Rt'], sigma_name=f'log_sigma_Rt:{idx}', nsamples=nsamples)
        ...
        if 'Rt' in expt.keys():
            ...
            if f'Rmax:{idx:02d}' in mcmc_trace.keys():
                f_prior_Rmax = vmap(lambda param: _uniform_pdf(param, 0.2*maxR, 10*maxR))
                log_priors += jnp.log(f_prior_Rmax(mcmc_trace[f'Rmax:{idx:02d}'][:nsamples]))

            if f'Rmax_P:{idx:02d}' in mcmc_trace.keys():
                f_prior_Rmax = vmap(lambda param: _uniform_pdf(param, 0.2*maxR, 10*maxR))
                log_priors += jnp.log(f_prior_Rmax(mcmc_trace[f'Rmax_P:{idx:02d}'][:nsamples]))

            if f'y_offset:{idx:02d}' in mcmc_trace.keys():
                f_prior_y_offset = vmap(lambda param: _uniform_pdf(param, minR-10, minR+10.))
                log_priors += jnp.log(f_prior_y_offset(mcmc_trace[f'y_offset:{idx:02d}'][:nsamples]))

    return np.array(log_priors)


def _log_likelihood_each_dataset(expt, trace_ka, trace_kd, trace_Rmax, 
                                 trace_rate_decay, trace_y_offset, trace_conc, trace_sigma, 
                                 nsamples):
    """
    Parameters:
    ----------
    ...
    f_Rt = vmap(lambda ka, kd, Rmax, rate, y_offset, conc, sigma: _log_likelihood_normal(Rt[to_fit],
                                                                                         jnp.array(jnp.cumsum(f_dRdt(ka, kd, Rmax, rate, y_offset, conc))*jnp.insert(jnp.diff(ts), 0, 0))[to_fit],
                                                                                         sigma))
    log_likelihoods += f_Rt(trace_ka, trace_kd, trace_Rmax, trace_rate_decay, trace_y_offset, trace_conc, trace_sigma)
...
def _log_likelihoods(mcmc_trace, experiments, nsamples=None, show_progress=True):
    ...
    for idx, expt in enumerate(experiments):
        ...
        log_likelihoods += _log_likelihood_each_dataset(expt=expt, trace_ka=ka, trace_kd=kd, trace_Rmax=Rmax, 
                                                        trace_rate_decay=rate_decay, trace_y_offset=y_offset, trace_conc=conc_uM, 
                                                        trace_sigma=sigma, nsamples=nsamples)

    return np.array(log_likelihoods)


def _log_likelihood_each_dataset_complex(expt, trace_ka_P, trace_kd_P, trace_Rmax_P, 
                                         trace_ka_C, trace_kd_C, trace_Rmax_C, 
                                         trace_rate_decay, trace_alpha, trace_epsilon,
                                         trace_y_offset, trace_conc, trace_sigma, nsamples):
    """
    Parameters:
    ----------
    expt          : list of multiple expts, each expt contain information about the data
    ...
    f_Rt = vmap(lambda ka_P, kd_P, Rmax_P, ka_C, kd_C, Rmax_C, rate, alpha, epsilon, y_offset, conc, sigma: _log_likelihood_normal(Rt[to_fit],
                                                                                                                                   jnp.array(jnp.cumsum(f_dRdt_PL(ka_P, kd_P, Rmax_P, rate, epsilon, y_offset, conc))*jnp.insert(jnp.diff(ts),0,0))[to_fit] 
                                                                                                                                        + alpha*R_CL[to_fit], #+ alpha*jnp.array(jnp.cumsum(f_dRdt_CL(ka_C, kd_C, Rmax_C, conc))*jnp.insert(jnp.diff(ts),0,0))[to_fit],
                                                                                                                                   sigma))
    log_likelihoods += f_Rt(trace_ka_P, trace_kd_P, trace_Rmax_P, trace_ka_C, trace_kd_C, trace_Rmax_C, 
                            trace_rate_decay, trace_alpha, trace_epsilon, trace_y_offset, trace_conc, trace_sigma)

    return log_likelihoods


def _log_likelihoods_complex(mcmc_trace, experiments, nsamples=None, show_progress=True):
    """
    Sum of log likelihood of all parameters given their distribution information in params_dist
    ...
    for idx, expt in enumerate(experiments):
        ...
        if sigma is not None:
            if expt['binding_type'] == 'specific':
                log_likelihoods += _log_likelihood_each_dataset_complex(expt=filtered_experiments[idx], 
                                                                        trace_ka_P=ka_P, trace_kd_P=kd_P, trace_Rmax_P=Rmax_P,
                                                                        trace_ka_C=ka_C, trace_kd_C=kd_C, trace_Rmax_C=Rmax_C, 
                                                                        trace_rate_decay=rate_decay, trace_alpha=alpha, trace_epsilon=epsilon, trace_y_offset=y_offset, 
                                                                        trace_conc=conc_uM, trace_sigma=sigma, nsamples=nsamples)
            else:
                log_likelihoods += _log_likelihood_each_dataset(expt=filtered_experiments[idx], trace_ka=ka_C, trace_kd=kd_C, trace_Rmax=Rmax_C, 
                                                                trace_rate_decay=rate_decay, trace_y_offset=y_offset, trace_conc=conc_uM, trace_sigma=sigma, 
                                                                nsamples=nsamples)

    return np.array(log_likelihoods)


def map_finding(mcmc_trace, experiments, fitting_complex=False, nsamples=None, show_progress=True):
    """
    ...
    log_priors = _log_priors(mcmc_trace=mcmc_trace, experiments=experiments, nsamples=nsamples)
    ...
    log_likelihoods = f_log_likelihoods(mcmc_trace=mcmc_trace, experiments=experiments,
