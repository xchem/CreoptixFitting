...
def gaussian_ci_from_sample(sample, level, bootstrap_repeats=1000):
    ...
    assert 0.0 < level < 1.0, "level must be 0 < level < 1"

    level *= 100
    l_percentile = (100.0 - level) / 2
    u_percentile = 100.0 - l_percentile
    ...
    lower_error = np.std(
        [
            np.percentile(
                np.random.choice(sample, size=sample.shape[0], replace=True),
                l_percentile,
            )
...
def bayesian_credible_interval(sample, level, bootstrap_repeats=1000):
    ...
    lower, upper = az.hdi(np.array(sample), hdi_prob=level)

    lowers = []
    uppers = []
    for _ in range(bootstrap_repeats):
        l, u = az.hdi(
            np.random.choice(sample, size=sample.shape[0], replace=True), level
        )
        lowers.append(l)
        uppers.append(u)
...
def _containing_rate(lowers, uppers, test_val):
    ...
    if isinstance(test_val, np.ndarray):
        ...
        rate = np.mean(
            [
                _contains_or_not(lower, upper, test_val)
...
def rate_of_containing_from_means_stds(
    means, stds, level, estimate_of_true="median", true_val=None
):
    """
    means   :   list of float
    stds    :   list of float
    ...
    return
            rate    : float
    """
    assert estimate_of_true in [
        "mean",
        "median",
    ], "estimate_of_true must be either 'mean' or 'median'"
...
def rate_of_containing_from_sample(
    samples,
    level,
    ...
    assert ci_type in [
        "bayesian",
        "gaussian",
    ], "ci_type must be either 'bayesian' or 'gaussian'"
    ...
    for sample in samples:
        ...
        if ci_type == "gaussian":
            lower, upper, _, _ = gaussian_ci_from_sample(
                sample, level, bootstrap_repeats=1
            )

        elif ci_type == "bayesian":
            lower, upper, _, _ = bayesian_credible_interval(
                sample, level, bootstrap_repeats=1
            )
    ...
    for _ in range(bootstrap_repeats):
        ...
        for sample in samples:
            ...
            if ci_type == "gaussian":
                lower, upper, _, _ = gaussian_ci_from_sample(
                    rand_sample, level, bootstrap_repeats=1
                )
...
def ci_convergence(repeated_samples, list_of_stops, level, ci_type="bayesian"):
    ...
    assert isinstance(
        list_of_stops, (list, np.ndarray)
    ), "list_of_stops must be a list or ndarray"
    ...
    for i, sample in enumerate(repeated_samples):
        for j, stop in enumerate(list_of_stops):
            ...
            if ci_type == "bayesian":
                l, u, _, _ = bayesian_credible_interval(
                    sub_sample, level, bootstrap_repeats=1
                )
            elif ci_type == "gaussian":
                l, u, _, _ = gaussian_ci_from_sample(
                    sub_sample, level, bootstrap_repeats=1
                )
