...
def plot_vertically_stacked_cis(
    lowers,
    uppers,
    xlabel,
    out,
    ...
    if (lower_errors is not None) and (upper_errors is not None):
        assert (
            len(lower_errors) == len(upper_errors) == len(lowers)
        ), "lowers, lower_errors and upper_errors must have the same len"
...
def plot_containing_rates(
    ...
    assert len(predicted_rates) == len(
        observed_rates
    ), "predicted_rates and observed_rates do not have the same len"

    if observed_rate_errors is not None:
        assert len(observed_rates) == len(
            observed_rate_errors
        ), "observed_rates and observed_rate_errors do not have the same len"
    else:
    ...
    if show_diagonal_line:
        ax.plot(
            xlimits,
            ylimits,
            linestyle=diagonal_line_style,
            linewidth=diagonal_line_w,
            color=diagonal_line_c,
        )
...
def plot_ci_convergence(
    ...
    assert lowers.shape[-1] == len(
        list_of_stops
    ), "lowers.shape[-1] must be the same as len(list_of_stops)"
    ...
    for repeat in range(nrepeats):
        plt.plot(
            list_of_stops,
            lowers[repeat],
            linestyle=repeats_linestyle,
            color=repeats_colors[repeat],
            linewidth=repeats_lw,
            alpha=repeats_alpha,
        )
        plt.plot(
            list_of_stops,
